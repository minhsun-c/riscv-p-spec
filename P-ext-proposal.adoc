...
The uimm3 field bits [2:0] encode the 3-bit shift amount for byte elements.
...
amount encoded in uimm3[2:0] and writes the packed byte result to `rd`. Bits
...
The uimm4 field bits [3:0] encode the 4-bit shift amount for halfword elements.
...
amount encoded in uimm4[3:0] and writes the packed halfword result to `rd`.
Bits shifted out are discarded and vacated bit positions are filled with zeros.
...
The uimm5 field bits [4:0] encode the 5-bit shift amount for word elements.
...
amount encoded in uimm5[4:0] and writes the packed word result to `rd`. Bits
shifted out are discarded and vacated bit positions are filled with zeros.
Available only in RV64.
...
immediate shift amount encoded in uimm3[2:0] and writes the packed byte result
...
immediate shift amount encoded in uimm4[3:0] and writes the packed halfword
...
immediate shift amount encoded in uimm5[4:0] and writes the packed word result
to `rd`. Bits shifted out are discarded and vacated bit positions are filled
...
the immediate shift amount encoded in uimm4[3:0] with rounding and writes the
...
the immediate shift amount encoded in uimm5[4:0] with rounding and writes the
...
The uimm5 field bits [4:0] encode the 5-bit shift amount.
...
The uimm5 field bits [4:0] encode the 5-bit shift amount for RV32.
...
The uimm6 field bits [5:0] encode the 6-bit shift amount for RV64.
